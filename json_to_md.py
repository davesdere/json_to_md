#-*- coding: utf-8 -*-
# Inspired by https://github.com/PolBaladas/torsimany
import sys
import json
import argparse
...
def parse_dict(d, depth, options):
    for k in d:
        if k in options['ignore']: continue
        if options['keep'] != '':
            if k not in options['keep']: continue
...
def parse_list(l, depth, options):
    for value in l:
...
def main():
    parser = argparse.ArgumentParser(description="Json to Markdown converter", 
                                    usage='%(prog)s -i $INPUTFILENAME [options]',
                                    epilog="Ca va bien aller!")
